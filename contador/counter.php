...
//$count = $db->query('SELECT COUNT() AS visitors, COUNT(DISTINCT page_url) AS pages FROM online WHERE page_url = :estaurl')->fetch(PDO::FETCH_ASSOC);
$counter = $db->prepare('SELECT COUNT() AS visitors, COUNT(DISTINCT page_url) AS pages FROM online WHERE page_url = :estaurl');
$counter->bindValue(':estaurl', $page_url, PDO::PARAM_STR);
$counter->execute();
//$resultado = $counter->get_result();
$count = $counter->fetch(PDO::FETCH_ASSOC);
